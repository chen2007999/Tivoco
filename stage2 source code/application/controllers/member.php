...
class Member extends CI_Controller {
    ...
    public function activate($activation_code = "")
    {
        ...
        $data = array(
            ...
            'page_view' => 'web/pages/pg-dashboard'
        );
    ...
    private function popup_signin_signin_view()
    {
        $data = array(
            'page_title' => "Sing In",
    ...
    private function create_session($user_name='',$password='')
    {
        ...
        if($mem_data)
        {
            if($mem_data->status == "Pending Confirmation")
            {
                $this->session->set_flashdata('response', '<div class="error-box-home">Your account has not been verified yet.</div>');
                redirect($_SERVER['HTTP_REFERER'],'refresh');
            }
            ...
            else if($mem_data->status == "Active")
            {
                ...
                if($this->member_model->is_profile_completed())
                {
                    ...
                    <script>
                        ...
                        parent.window.location='<?PHP echo base_url()."member/dashboard";?>'</script>
                    <?PHP
                }
                else
                {
                    ...
                    <script>
                        ...
                        parent.window.location='<?PHP echo base_url()."member/dashboard/true";?>'</script>
                    <?PHP
                }
    ...
    public function dashboard($profile_popup = false)
    {
        $data = array(
    ...
    {
        $this->load->view('web/pages/pg-dashboard');
    }
    ...
    private function upload($field = 'image')
    {
        $path = './uploads';
    ...
    private function update_member_profile_photo()
    {
        $flag = 0;
        ...
        if($mem_data->photo == "")
        {
            if($this->upload())
            {
                ...
                $this->db_model->update_row('members',array('photo'=>$photo_data['file_name']),array('id'=>$this->session->userdata('member_data')->id));
            }
            else
            {
    ...
    public function profile()
    {
        ...
        if($this->input->post())
        {
            ...
            $this->form_validation->set_rules('description','Description/Detail','required');

            $vals = $this->input->post();

            if ($this->form_validation->run() == TRUE &&  $this->update_member_profile_photo() == TRUE)
            {

                unset($vals['btnSubmit'],$vals['user_name'],$vals['email']);

                $vals['last_modified'] = date('Y-m-d h:i:s');
                $vals['is_profile_complete'] = 'Yes';
    ...
    {
        $this->load->view('web/includes/friends-list');
    }

    public function logout()
    {
        $this->db_model->update_row('members',array('online'=>'No'),array('id'=>$this->session->userdata('member_data')->id));
